...
async function getFileList(uid, pass, torrentId) {
    ...
    // Zamunda shows files in a table or list, look for common patterns
    // Pattern 1: table rows with filenames
    $('td.heading:contains("Файл"), td.heading:contains("File"), td:contains("Съдържание")').each((i, heading) => {
        // Find the file list near this heading
        const container = $(heading).closest('table').length ? $(heading).closest('table') : $(heading).parent().parent();
        container.find('td').each((j, td) => {
            const text = $(td).text().trim();
            // Match video file patterns
            const fileMatch = text.match(/(.+\.(mkv|mp4|avi|wmv|flv|mov|m4v|ts|webm))/i);
            if (fileMatch) {
                files.push({
    ...
    // Pattern 2: If no files found yet, try a broader search for video filenames
    if (files.length === 0) {
        const allText = $.html();
        // Find all video filenames in the page
        const fileRegex = /([^\s<>"]+\.(mkv|mp4|avi|wmv|flv|mov|m4v|ts|webm))/gi;
        let match;
        const seen = new Set();
        while ((match = fileRegex.exec(allText)) !== null) {
            const fileName = match[1].replace(/.*[\/\\]/, ''); // Get just filename
    ...
    // Pattern 3: Look for structured file list (common in Zamunda)
    if (files.length === 0) {
        $('pre, code, .filelist, #filelist').each((i, elem) => {
            const text = $(elem).text();
            const lines = text.split('\n');
            for (const line of lines) {
                const fileMatch = line.match(/(.+\.(mkv|mp4|avi|wmv|flv|mov|m4v|ts|webm))/i);
                if (fileMatch) {
                    files.push({
                        name: fileMatch[1].trim(),
                        index: fileIndex++
    ...
    // Pattern 4: Raw HTML – any string ending with video extension (Zamunda may embed in JS/attributes)
    if (files.length === 0) {
        const raw = html.replace(/&#(\d+);/g, (_, n) => String.fromCharCode(parseInt(n, 10)));
        const fileRegex = /[\w\u0400-\u04FF.\-\s]{3,150}\.(mkv|mp4|avi|wmv|flv|mov|m4v|ts|webm)(?=[\s"'<>)\]}]|$)/gi;
        let m;
        const seenRaw = new Set();
        while ((m = fileRegex.exec(raw)) !== null) {
            const name = m[0].trim().replace(/^.*[\/\\]/, '');
            if (name.length < 4 || seenRaw.has(name.toLowerCase())) continue;
            seenRaw.add(name.toLowerCase());
            files.push({ name, index: files.length });
        }
    }

    // Pattern 5: Quoted filenames in HTML/JS ("file.mkv" or 'file.mkv')
    if (files.length === 0) {
        const quoted = html.match(/["']([^"']*\.(?:mkv|mp4|avi|wmv|flv|mov|m4v|ts|webm))["']/gi);
        if (quoted) {
            const seenQ = new Set();
            quoted.forEach(s => {
                const name = s.replace(/^["']|["']$/g, '').replace(/^.*[\/\\]/, '');
                if (name.length > 3 && !seenQ.has(name.toLowerCase())) {
                    seenQ.add(name.toLowerCase());
                    files.push({ name, index: files.length });
                }
            });
        }
    ...
    for (const result of filtered.slice(0, 10)) {
        try {
            ...
            if (type === 'series' && filter?.season && filter?.episode && isSeasonPack(result.title)) {
                console.log(`[Streams] Season pack detected: "${result.title}" — looking for S${String(filter.season).padStart(2,'0')}E${String(filter.episode).padStart(2,'0')}...`);
                try {
                    ...
                    if (fileIdx !== null) {
                        ...
                        stream.title = `${result.title.substring(0, 60)} [Ep ${filter.episode}]\n👤 ${result.seeders}\n📁 ${result.size}\n🌐 Zamunda`;
                        console.log(`[Streams] Set fileIdx=${fileIdx} for episode S${String(filter.season).padStart(2,'0')}E${String(filter.episode).padStart(2,'0')}`);
                    } else {
                        console.log(`[Streams] Could not find episode file, skipping season pack`);
                        continue;
                    }
                } catch (e) {
