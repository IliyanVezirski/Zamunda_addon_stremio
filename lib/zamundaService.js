...
const { getSession, createClient, ZAMUNDA_URL } = require('./sessionManager');
...
async function loginAndGetCookies(username, password) {
    try {
        ...
        return session; // { uid, pass }
    } catch (e) {
        console.error('[Zamunda] Login failed:', e.message);
        return null;
    }
}
 ...
 */
async function search(cookies, query) {
    const client = createClient(cookies.uid, cookies.pass);
 ...
 */
async function getMagnet(cookies, torrentId) {
    const client = createClient(cookies.uid, cookies.pass);
 ...
 */
async function getStreams(cookies, query, type) {
    const results = await search(cookies, query);
    const streams = [];

    for (const result of results.slice(0, 10)) {
        try {
            const magnet = await getMagnet(cookies, result.id);
            if (!magnet) continue;
...
module.exports = { loginAndGetCookies, getStreams };
