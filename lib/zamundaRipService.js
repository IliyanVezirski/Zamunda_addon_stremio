...
async function getStreams(query, type, filter) {
    ...
    console.log(`[ZamundaRIP] ${results.length} results for "${filter?.name}" (${filter?.year || '?'})`);
    ...
    for (const result of results.slice(0, 10)) {
        try {
            ...
            // Add public trackers as fallback (critical for TV clients)
    ...
    const finalStreams = streams.map((s, i) => {
        ...
        return {
            ...
            infoHash: s.infoHash,
            sources: s.sources.map(t => t.startsWith('tracker:') ? t : 'tracker:' + t),
