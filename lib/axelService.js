...
async function search(uid, pass, query) {
    ...
    const res = await axios.get(WORKER_URL, {
        params: { target: 'axelbg.net', path, cookies },
        timeout: 20000
    });

    const html = typeof res.data === 'string' ? res.data : JSON.stringify(res.data);
    console.log(`[Axel] Response: ${html.length} chars, type=${typeof res.data}, has details.php=${html.includes('details.php')}, has download.php=${html.includes('download.php')}, has logout=${html.includes('logout')}`);
    if (html.length < 1000) {
...
async function getTorrentInfo(uid, pass, downloadHref) {
    ...
    const path = downloadHref.startsWith('/') ? downloadHref : `/${downloadHref}`;
    const cookies = `uid=${uid}; pass=${pass}`;

    console.log(`[Axel] Downloading .torrent via Worker: ${path.substring(0, 80)}...`);
    const res = await axios.get(WORKER_URL, {
        params: { target: 'axelbg.net', path, cookies, binary: '1' },
        timeout: 20000,
