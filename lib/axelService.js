...
async function search(uid, pass, query) {
    ...
    console.log(`[Axel] Response: status=${res.status}, colo=${colo}, ${html.length} chars, has details.php=${html.includes('details.php')}, has logout=${html.includes('logout')}`);
    
    if (res.status >= 400) {
        console.log(`[Axel] Error body (first 300): ${html.substring(0, 300)}`);
        return [];
    }
