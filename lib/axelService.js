...
async function search(uid, pass, query) {
    ...
    // Retry up to 3 times — axelbg.net gives SQL Error from FRA/DUS edges, only SOF works
    let html = '';
    let lastColo = 'unknown';
    for (let attempt = 1; attempt <= 3; attempt++) {
        const res = await axios.get(workerUrl, { timeout: 20000, validateStatus: () => true });
        lastColo = res.headers?.['x-worker-colo'] || 'unknown';
        html = typeof res.data === 'string' ? res.data : JSON.stringify(res.data);
        console.log(`[Axel] Attempt ${attempt}: status=${res.status}, colo=${lastColo}, ${html.length} chars`);

        if (res.status >= 400) {
            console.log(`[Axel] Error status ${res.status}`);
            return [];
        }

        if (!html.includes('SQL Error')) break;
        console.log(`[Axel] SQL Error from ${lastColo}, retrying...`);
        await new Promise(r => setTimeout(r, 500 * attempt));
    }

    if (html.includes('SQL Error')) {
        console.log(`[Axel] SQL Error persists after 3 retries (last colo=${lastColo})`);
        return [];
    }
