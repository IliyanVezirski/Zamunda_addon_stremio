...
async function search(uid, pass, query) {
    ...
    console.log(`[Axel] Search: "${query}" — no cache, calling via Worker...`);
    const cookies = `uid=${uid}; pass=${pass}`;
    const path = `/browse.php?search=${encodeURIComponent(query)}&cat=0&incldead=0`;

    // Build URL manually — double-encode path so Worker receives it as single param
    const encodedPath = encodeURIComponent(path);
    const encodedCookies = encodeURIComponent(cookies);
    const workerUrl = `${WORKER_URL}/?target=axelbg.net&path=${encodedPath}&cookies=${encodedCookies}`;
    console.log(`[Axel] Full Worker URL: ${workerUrl}`);
    const res = await axios.get(workerUrl, { timeout: 20000, validateStatus: () => true });

    console.log(`[Axel] Worker status: ${res.status}, worker-version: ${res.headers?.['x-worker-version'] || 'MISSING'}, worker-path: ${res.headers?.['x-worker-path'] || 'MISSING'}`);
    const html = typeof res.data === 'string' ? res.data : JSON.stringify(res.data);
    console.log(`[Axel] Response: ${html.length} chars, has details.php=${html.includes('details.php')}, has logout=${html.includes('logout')}`);
    if (html.length < 1000) {
        console.log(`[Axel] Short response body: ${html.substring(0, 500)}`);
    }
    
    if (res.status >= 400) {
        console.log(`[Axel] Worker returned error status ${res.status}`);
        return [];
    }
