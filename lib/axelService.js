...
async function search(uid, pass, query) {
    ...
    console.log(`[Axel] Search: "${query}" — no cache, calling axelbg.net...`);
    const cookies = `uid=${uid}; pass=${pass}`;

    const res = await axios.get(`${AXEL_BASE}/browse.php?search=${encodeURIComponent(query)}&cat=0&incldead=0`, {
...
async function getTorrentInfo(uid, pass, downloadHref) {
    ...
    const dlUrl = downloadHref.startsWith('http') ? downloadHref : `${AXEL_BASE}/${downloadHref}`;
    const cookies = `uid=${uid}; pass=${pass}`;

    console.log(`[Axel] Downloading .torrent: ${dlUrl.substring(0, 80)}...`);
