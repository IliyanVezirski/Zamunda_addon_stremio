...
async function search(uid, pass, query) {
    ...
    const path = `/browse.php?search=${encodeURIComponent(query)}&cat=0&incldead=0&page=0`;

    const workerUrl = `${WORKER_URL}/?target=axelbg.net&path=${encodeURIComponent(path)}&cookies=${encodeURIComponent(cookies)}`;
    console.log(`[Axel] Worker URL: ${workerUrl.substring(0, 150)}...`);
    const res = await axios.get(workerUrl, { timeout: 20000, validateStatus: () => true });

    const colo = res.headers?.['x-worker-colo'] || 'unknown';
    const workerPath = res.headers?.['x-worker-path'] || 'missing';
    const html = typeof res.data === 'string' ? res.data : JSON.stringify(res.data);
    console.log(`[Axel] Response: status=${res.status}, colo=${colo}, workerPath=${workerPath}, ${html.length} chars`);
    
    if (html.length < 2000) {
        console.log(`[Axel] Short body (first 400): ${html.substring(0, 400)}`);
    }
